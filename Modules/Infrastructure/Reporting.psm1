...
function Invoke-WRAReporting {
    ...
    # ----- Scores -----
    $scores = $null
    if (Get-Command -Name 'Get-WRAScores' -ErrorAction SilentlyContinue) {
        try { $scores = Get-WRAScores -Results $resultsArr -Config $Config } catch { }
    }
    ...
    $dataset = [PSCustomObject]@{
        meta = [PSCustomObject]@{
            ...
            version        = [string](Get-WRAProp -Object $Config -Path 'Version' -Default '4.1.0')
