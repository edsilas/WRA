...
function ConvertTo-WRAIsoDate {
    # Converte um valor de data para texto ISO 8601 ('o'). Motivo: o
    # ConvertTo-Json do Windows PowerShell 5.1 serializa objetos DateTime no
    # formato legado "/Date(ms)/", ilegivel no dashboard. Em caso de falha,
    # devolve o valor original (nunca lanca excecao).
...
function Get-WRAMonitorServices {
    ...
    # Sem -Property: obtem o objeto completo. 'DelayedAutoStart' NAO existe no
    # Win32_Service de sistemas anteriores ao Windows 10/Server 2016 e, em WQL,
    # selecionar uma propriedade inexistente invalida a consulta inteira
    # (zerando toda a secao de servicos). Com o objeto completo, a leitura e
    # feita com seguranca via Get-WRAProp (default $false quando ausente).
    $svc = @(Invoke-WRAMonitorCim -ClassName 'Win32_Service' -TimeoutSec $TimeoutSec -Warnings $Warnings)
    ...
    foreach ($s in $svc) {
        ...
        if ($start -eq 'Auto' -and $state -ne 'Running') {
            ...
            # ExitCode e uint32 e pode exceder Int32 (ex.: 3489660935); comparamos
            # sem forcar [int] para evitar estouro de conversao. Qualquer valor
            # nao numerico ou diferente de zero e tratado como saida nao limpa.
            $exitCode = Get-WRAProp -Object $s -Path 'ExitCode' -Default $null
            $cleanExit = $false
            if ($null -ne $exitCode) {
                $exitNum = 0.0
                if ([double]::TryParse([string]$exitCode, [ref] $exitNum)) { $cleanExit = ($exitNum -eq 0.0) }
            }
        ...
        foreach ($e in $events) {
            ...
            if ($items.Count -lt $MaxItems) {
                ...
                [void]$items.Add([PSCustomObject]@{
                    TimeCreated = (ConvertTo-WRAIsoDate -Value (Get-WRAProp -Object $e -Path 'TimeCreated' -Default $null))
...
function Get-WRAMonitorEventAnalysis {
    ...
    foreach ($wrap in $raw) {
        ...
        # ISO desde a origem: comparacoes continuam via cast [datetime], e o
        # payload sai legivel/ordenavel (sem o formato legado "/Date(ms)/").
        $tc = ConvertTo-WRAIsoDate -Value (Get-WRAProp -Object $e -Path 'TimeCreated' -Default $null)
        if ($null -ne $tc) {
...
if (Get-Command -Name 'New-WRAModuleManifest' -ErrorAction SilentlyContinue) {
    ...
    $WRAMonitorManifest = New-WRAModuleManifest -Module 'Monitor' -Operations $ops `
        -Version '1.1.0' -Description 'Monitoramento amostrado de recursos do sistema.'
}
