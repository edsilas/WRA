...
function Get-WRAInvLicensing {
    ...
    $prods = @(Invoke-WRAInvCim -ClassName 'SoftwareLicensingProduct' -Property @('Name', 'ApplicationID', 'LicenseStatus', 'PartialProductKey', 'Description') -TimeoutSec $TimeoutSec -Warnings $null -Quiet)
    $activation = 'Unknown'
    $name = ''
    foreach ($p in $prods) {
        $appId = [string](Get-WRAProp -Object $p -Path 'ApplicationID' -Default '')
        $ppk = [string](Get-WRAProp -Object $p -Path 'PartialProductKey' -Default '')
        if ($appId -eq $winAppId -and $ppk) {
    ...
    return [PSCustomObject]@{ WindowsActivation = $activation; LicenseName = $name }
}
...
if (Get-Command -Name 'New-WRAModuleManifest' -ErrorAction SilentlyContinue) {
    ...
    $WRAInvManifest = New-WRAModuleManifest -Module 'Inventory' -Operations $ops `
        -Version '4.1.0' -Description 'Inventario detalhado do sistema.'
}
