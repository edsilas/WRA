...
function Get-WRASeverityPoints {
...
function Invoke-WRASecurityAudit {
    ...
    # ----- Calculo de scores -----
    $weights = @{
        Defender      = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.Defender' -Default 0.25)
        Firewall      = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.Firewall' -Default 0.2)
        WindowsUpdate = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.WindowsUpdate' -Default 0.2)
        BitLocker     = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.BitLocker' -Default 0.15)
        SecureBoot    = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.SecureBoot' -Default 0.1)
        Uac           = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.Weights.Uac' -Default 0.1)
    }
    $uncertain = [double](Get-WRAProp -Object $config -Path 'Scoring.Security.UncertainSubScore' -Default 0.5)
    $num = 0.0; $den = 0.0; $indetW = 0.0
    foreach ($k in $weights.Keys) {
        if ($weighted.ContainsKey($k)) {
            ...
            if ($chk.Applicable) {
                $num += $weights[$k] * [double]$chk.SubScore
                $den += $weights[$k]
            }
            elseif (-not $determinable) {
                # Controle-chave nao verificavel: conta como incerto para nao inflar
                # o score, e registra a lacuna como recomendacao (surface o desconhecido).
                $num += $weights[$k] * $uncertain
                $den += $weights[$k]
                $indetW += $weights[$k]
            ...
            # N/A legitimo (Applicable=false, Determinable=true): excluido do calculo.
        }
    }
    $securityScore = 0
    $secCoverage = $null
    if ($den -gt 0) {
        $securityScore = [Math]::Round(($num / $den) * 100, 1)
        $secCoverage = [Math]::Round(($den - $indetW) / $den, 3)
    }
    $secDeterminable = ($null -ne $secCoverage -and $secCoverage -ge 0.5)

    $riskScale = [int](Get-WRAProp -Object $config -Path 'Scoring.Risk.Scale' -Default 100)
    $riskPoints = 0
    $critFindings = 0; $highFindings = 0
    foreach ($r in $recs) {
        $sev = [string](Get-WRAProp -Object $r -Path 'Severity' -Default 'Low')
        $riskPoints += (Get-WRASeverityPoints -Severity $sev)
        if ($sev -eq 'Critical') { $critFindings++ }
        elseif ($sev -eq 'High') { $highFindings++ }
    }
    $riskScore = [Math]::Min($riskScale, $riskPoints)

    $healthScore = [Math]::Max(0, $securityScore - (5 * $critFindings) - (2 * $highFindings))
    ...
    $data = [PSCustomObject]@{
        Scores = [PSCustomObject]@{
            ...
            Determinable  = $secDeterminable
            Scope         = 'Dominio de seguranca. O Health Score global e composto na geracao de relatorios.'
        }
...
if (Get-Command -Name 'New-WRAModuleManifest' -ErrorAction SilentlyContinue) {
    ...
    $WRASecManifest = New-WRAModuleManifest -Module 'Security' -Operations $ops -RequiresElevation `
        -Version '4.1.0' -Description 'Auditoria de seguranca somente leitura (nunca altera configuracoes).'
}
