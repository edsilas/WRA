...
Describe 'Integration (live)' {
    ...
    It 'Executa a auditoria All e compoe os scores' -Skip:$skip {
        ...
        $scores = Get-WRAScores -Results $results -Config $cfgObj
        Assert-NotNull $scores
        if ($null -ne $scores.Health) {
            Assert-True ($scores.Health -ge 0 -and $scores.Health -le 100) 'Health fora de faixa'
        }
    }
}
